...
import { getValidKVSRecordKey, IRequestResponse, lotSizeToString, sqft2acre, START_TIMESTAMP } from './utils'
import { REDFIN } from './utils/redfin'
...
export const handleLocation = async (
    ...
    const placeType = (REDFIN.PLACE_TYPE as any)[searchType.toLowerCase()]

    const json = JSON.parse((response.body as any)?.replace('{}&&', ''))
    const locations = json?.payload?.sections?.[0]?.rows?.filter((s: any) => s.type === `${placeType}`)
    let data = locations[0]
    if (searchType === 'county') {
        data = locations.find((l: any) => `${l.name}, ${l.subName}` === `${query}, USA`)
    }
    ...
    const regionType = (REDFIN.REGION_TYPE as any)[searchType.toLowerCase()]
