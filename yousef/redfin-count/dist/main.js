...
async function main() {
    ...
    if (!ignoreStartRequests) {
        ...
        if (location) {
            ...
            const requestConfig = (0, custom_utils_1.getRequestConfig)(custom_utils_1.DESTINATION.LOCATION, finalInput);
            const userData = requestConfig.userData;
            requests.push({
                ...lodash_1.default.omit(requestConfig, ['userData']),
                userData: {
                    label: custom_utils_1.LABELS.LOCATION,
                    ...userData
                }
            });
        }
    ...
    const sessions = await (0, utils_1.getSessionsUsingInput)(finalInput, 'redfin-count-sessions');
    ...
    const crawler = new crawlee_1.BasicCrawler({
                ...
                try {
                    ...
                    if (globalContext.shared.sessions.length) {
                        ...
                        ({ proxyUrl, requestHeaders, cookie, creationTime } =
                            aSession ??
                                (await (0, utils_1.getSession)(globalContext, sessionLog, 'https://www.redfin.com/', custom_utils_1.isRequestBlocked, forceCleanSessionsCreation)));
                        !Number.isNaN(sessionId) && (globalContext.shared.sessions[sessionId].inUse = true);
                    }
                    else {
                        ;
                        ({ proxyUrl, cookie, requestHeaders, creationTime } = await (0, utils_1.getSession)(globalContext, sessionLog, 'https://www.redfin.com/', custom_utils_1.isRequestBlocked, forceCleanSessionsCreation));
                    }
        ...
        requestHandler: async (crawlingContext) => {
            ...
            // }
            switch (label) {
                case custom_utils_1.LABELS.LOCATION:
