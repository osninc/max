...
const fixListings = (listings, details) => {
    const f = listings.map((listing) => {
        ...
        const newImage =
            listing.imgSrc.includes('googleapis.com') || listing.imgSrc === '' ? '/no-image.png' : listing.imgSrc;
        const newId = listing.zpid ?? listing.id;

        let secondaryDetails = {};
        // find listings // There are some records that has no zpid
        if (details)
            if (newId) secondaryDetails = details[newId];
            else
                secondaryDetails = {
                    dom: 'N/A',
                    views: 'N/A',
...
const checkCombine = (source, data, field) => {
    ...
    if (field.toUpperCase() == 'STATUS') {
        actualFieldValue =
            actualFieldValue.toLowerCase() === 'sold' ? data?.soldInLast?.toLowerCase() : data?.daysOn?.toLowerCase();
    }
...
export const normalizeTheData = (source, data, details) => {
    ...
    if (details) fixedDetails = fixDetails(details);
    ...
    data.map((count) => {
        if (count.status !== undefined) {
            const timeDim =
                count.status?.toLowerCase() === 'sold'
                    ? count?.soldInLast?.toLowerCase()
                    : count?.daysOn?.toLowerCase();
            ...
            if (timeDim) {
                ...
                const listingsWithValues = {
                    dom: fixedListings
                        ...
                        .filter((el) => el),
                    views: fixedListings
                        .filter((l) => l.views !== 'N/A')
                        .map((listing) => listing.views)
                        .filter((el) => el),
                    favorites: fixedListings
                        .filter((l) => l.favorites !== 'N/A')
                        .map((listing) => listing.favorites)
                        .filter((el) => el),
                };
                ...
                if (hit.acreage || hit.time) {
                    ...
                    const hasStatus =
                        hasAcreage &&
                        hasTimeDim &&
                        !(c[combined.acreage][combined.time][count.status.toLowerCase()] === undefined);
                    ...
                    c[combined.acreage] = {
                        ...
                        [combined.time]: {
                            ...
                            [count.status.toLowerCase()]: {
                                ...(hasStatus
                                    ? c[combined.acreage][combined.time][count.status.toLowerCase()]
                                    : defaultCalcFields),
                                ...combinedObj,
                            },
                        },
                    ...
                    c[count.acreage?.toLowerCase()] = {
                        ...
                        [timeDim.toLowerCase()]: {
                            ...(c[count.acreage?.toLowerCase()]
                                ? c[count.acreage?.toLowerCase()][timeDim?.toLowerCase()]
                                : {}),
                            timeStamp: count.timeStamp,
                            [count.status?.toLowerCase()]: {
