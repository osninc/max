...
const Marker = ({ children, listing, source }) => {
    ...
    const htmlTitle = (
        <Card>
            <CardMedia sx={{ height: 140 }} image={listing.imgSrc} title={listing.zpid} />
            <CardContent>
                ...
                <Typography variant="body2" color="text.secondary">
                    price: {USDollar.format(listing.unformattedPrice)}
                    <br />
                    acreage: {listing.acre} acres
                    <br />
                    price per acre: {USDollar.format(listing.unformattedPpa)}
                    <br />
    ...
    return (
        <Tooltip title={htmlTitle} placement="top">
            <IconButton
                ...
                href={getRealtorUrl(source, listing.zpid)}
                rel="noreferrer"
                target="_blank"
            >
...
export const MapView = ({ listings, onClick, source }) => {
    ...
    useEffect(() => {
        ...
        listings.map((listing) => {
            ...
            createRoot(ref.current).render(<Marker onClick={markerClicked} listing={listing} source={source} />);

            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            const f = new mapboxgl.Marker(ref.current)
