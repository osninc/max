...
export const toPascalCase = (str: string, replaceSpace = true) => {
    const result = _.startCase(_.toLower(str))
    if (!replaceSpace) {
        return result
    }
...
export const queryParametersToString = (options: {
    ...
    encodeKey?: boolean
    encodeValue?: boolean
    disperseValueAsArray?: boolean
}) => {
    const { queryParameters, encodeKey = true, encodeValue = true, disperseValueAsArray = false } = options
    return Array.from(Object.keys(queryParameters))
        .map((key) => {
            ...
            if (Array.isArray(qpValue) && disperseValueAsArray) {
                return qpValue
                    .map((qpValueArrayItem) => {
                        let qpValueFinalArrayItem = qpValueArrayItem

                        if (encodeValue) {
                            qpValueFinalArrayItem = encodeURIComponent(qpValueArrayItem)
                        }
                        return `${qpKey}=${qpValueFinalArrayItem}`
                    })
                    .join('&')
            }
